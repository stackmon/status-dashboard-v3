...
- Q: What is the JWT token claim structure for roles? How are `admin`, `creator`, and `operator` roles represented in the token? → A: Single 'groups' claim with array of IdP group names (e.g., ["admin-group", "sd_creators"]); each group name is matched against SD_RBAC_GROUPS_* env vars to resolve the application role
...
- **FR-003a**: System MUST map the `admin` role to the IdP group configured via SD_RBAC_GROUPS_ADMINS environment variable
