...
- Q: How should IdP group names be mapped to application roles (sd_creators, sd_operators, sd_admins) to avoid hardcoding group names in the codebase? → A: Environment variables map IdP group names to application roles (SD_RBAC_CREATORS, SD_RBAC_OPERATORS, SD_RBAC_ADMINS). RBAC must be explicitly enabled via SD_RBAC_ENABLED=true. Application reads JWT 'groups' claim and checks against configured env var values.
- Q: Role configuration mechanism: How are environment variables (SD_RBAC_CREATORS, SD_RBAC_OPERATORS, SD_RBAC_ADMINS) mapped to IdP groups? → A: Environment variables (SD_RBAC_CREATORS, SD_RBAC_OPERATORS, SD_RBAC_ADMINS) store IdP group names. When SD_RBAC_ENABLED=true, all three groups must be configured or the application fails to start. Application reads JWT 'groups' claim and matches against these configured values.
...
- **FR-002a**: System MUST map IdP group names to application roles using environment variables (SD_RBAC_CREATORS, SD_RBAC_OPERATORS, SD_RBAC_ADMINS). RBAC must be explicitly enabled via SD_RBAC_ENABLED=true; when enabled, all three group variables are required or the application MUST fail to start. The application reads the JWT 'groups' claim and checks if any value matches the configured environment variable values to determine role membership.
...
- **FR-003a**: System MUST map the sd_admins role to the IdP group configured via SD_RBAC_ADMINS environment variable
